...
const CHILDREN = Object.freeze([
  Object.freeze({ id: "lina-mansouri", initials: "LM", name: "Lina Mansouri", school: "École Al Manar", className: "5A", level: "A2", progress: 68, activity: "Aujourd’hui · 17 h 42", teacher: "Mme Salma Idrissi" }),
...
function ParentDashboard({ assignments, notifications }) {
  ...
  return <>
    ...
    <div className="parent-metric-grid"><MetricCard icon={Users} label="Enfants" value="2" note="Profils actifs" /><MetricCard icon={ClipboardText} label="Devoirs" value={String(assignments.length)} note="Publiés" tone="blue" /><MetricCard icon={TrendUp} label="Progression globale" value="68 %" note="+ 6 points" tone="coral" /><MetricCard icon={ChatCircleDots} label="Messages" value="4" note="1 non lu" tone="green" /></div>
    <div className="parent-dashboard-layout">
      <section className="panel parent-follow-panel" aria-labelledby="parent-follow-title"><div className="panel-heading"><h2 id="parent-follow-title">À accompagner cette semaine</h2><ParentLink to="/parent/devoirs">Voir tous les devoirs</ParentLink></div>
        <article className="parent-follow-row"><span className="parent-follow-icon danger"><ClipboardText weight="duotone" aria-hidden="true" /></span><div><h3>Consulter le dernier devoir</h3><p>{nextAssignment.title}</p></div><span className="status-pill danger">Nouveau</span><ParentLink to="/parent/devoirs" className="button button-light">Ouvrir <ArrowRight aria-hidden="true" /></ParentLink></article>
        ...
        <article className="parent-follow-row"><span className="parent-follow-icon info"><CalendarBlank weight="duotone" aria-hidden="true" /></span><div><h3>Voir la progression</h3><p>Bilan global · toutes les activités</p></div><span className="status-pill info">68 %</span><ParentLink to="/parent/progres" className="button button-light">Voir <ArrowRight aria-hidden="true" /></ParentLink></article>
      </section>
...
function ChildrenListPage({ search = "" }) {
  ...
  return <>
    ...
    <form id="parent-link-child" className="panel parent-link-child" onSubmit={linkChild}><div className="panel-heading"><div><h2>Relier un enfant</h2><p>Saisissez le code famille fourni par l’établissement.</p></div><LinkSimple aria-hidden="true" /></div><label>Code famille<input id="parent-family-code" defaultValue="JDE-5A-82K" required /></label><label>Prénom de l’enfant<input defaultValue="Lina" required /></label><label>Lien parental<select defaultValue="Parent"><option>Parent</option><option>Tuteur légal</option></select></label><label>Établissement<select defaultValue="École Al Manar"><option>École Al Manar</option></select></label><p className="parent-inline-info"><ShieldCheck aria-hidden="true" /> La demande doit être validée par l’établissement.</p><button className="button button-dark" type="submit"><LinkSimple aria-hidden="true" /> Relier le profil</button><p className="parent-form-status" role="status">{linkStatus}</p></form></div>
  </>;
}
...
function HomeworkPage({ assignments, search = "" }) {
  ...
  const list = [...source, ...FALLBACK_HOMEWORK].filter((item, index, array) => array.findIndex((candidate) => candidate.id === item.id) === index);
  const [filter, setFilter] = useState("todo");
  const [expanded, setExpanded] = useState(null);
  const normalizedSearch = search.trim().toLowerCase();
  const counts = {
    todo: list.filter(item => !["Terminé", "Corrigé"].includes(item.status)).length,
    done: list.filter(item => item.status === "Terminé").length,
    corrected: list.filter(item => item.status === "Corrigé").length,
    all: list.length,
  };
  const filtered = list
  ...
  return <>
    ...
    <div className="parent-homework-filters" role="tablist" aria-label="Filtrer les devoirs">{[["todo", "À faire", counts.todo], ["done", "Terminés", counts.done], ["all", "Tous", counts.all], ["corrected", "Corrigés", counts.corrected]].map(([key, label, count]) => <button key={key} type="button" role="tab" aria-selected={filter === key} className={filter === key ? "active" : ""} onClick={() => setFilter(key)}>{label} <span>{count}</span></button>)}</div>
    <div className="parent-homework-layout"><section className="parent-homework-list" aria-label="Devoirs publiés">{filtered.length ? filtered.map((item, index) => { const isOpen = expanded === item.id; const completed = ["Terminé", "Corrigé"].includes(item.status); const tone = index === 0 ? "oral" : completed ? "done" : "quiz"; return <article key={item.id} className={`parent-homework-card ${tone}`}><span className="parent-homework-type">{item.subject || "Français"}</span><div className="parent-homework-main"><div><span>{formatDueDate(item.dueAt)} · 18 h</span><span className={`status-pill ${completed ? "success" : "warning"}`}>{item.status || "À faire"}</span></div><h2>{item.title}</h2><p>{completed ? `${item.status} · ${item.duration || "9/10"}` : `À rendre · ${item.duration || "10 min"}`}</p>{isOpen && <div id={`parent-homework-${item.id}`} className="parent-homework-details"><strong>Consigne de l’enseignante</strong><p>{item.instructions || "Les consignes détaillées seront ajoutées par l’enseignante."}</p></div>}</div><button className={completed ? "button button-light" : "button button-green"} type="button" onClick={() => setExpanded(isOpen ? null : item.id)} aria-expanded={isOpen} aria-controls={`parent-homework-${item.id}`}>{isOpen ? "Masquer" : completed ? "Voir le résultat" : "Voir le devoir"} <ArrowRight aria-hidden="true" /></button></article>; }) : <div className="parent-list-empty"><ClipboardText weight="duotone" aria-hidden="true" /><h2>Aucun devoir à afficher</h2><p>Changez de filtre ou utilisez une autre recherche.</p></div>}<div className="parent-homework-reassurance"><CheckCircle weight="duotone" aria-hidden="true" /><span><strong>{counts.todo ? counts.todo + " devoir" + (counts.todo > 1 ? "s" : "") + " demande" + (counts.todo > 1 ? "nt" : "") + " votre attention" : "Lina est à jour"}</strong><small>Les échéances sont synchronisées avec l’espace enseignant.</small></span></div></section>
...
function MessagesPage({ notifications, notify, currentUser, search = "" }) {
  ...
  useEffect(() => {
    if (composeOpen) requestAnimationFrame(() => composeRef.current?.focus());
  }, [composeOpen]);
  const closeCompose = () => { setComposeOpen(false); requestAnimationFrame(() => composeButtonRef.current?.focus()); };
  ...
  const submit = (event) => { event.preventDefault(); const clean = draft.trim(); if (clean.length < 3) { setStatus("Écrivez un message d’au moins trois caractères."); return; } const result = notify({ role: "parent", userId: currentUser?.id || "user-parent-youssef", type: "message_parent", title: "Message à Mme Salma Idrissi", message: clean, action: "/parent/messages" }); if (result.ok) { setDraft(""); setStatus("Message enregistré sur cet appareil."); closeCompose(); } else setStatus(result.message || "Le message n’a pas pu être enregistré."); };
  return <>
    <PageHeader eyebrow="ÉCOLE–FAMILLE" title="Messages" subtitle="Échangez avec l’équipe pédagogique et l’administration de l’école." action={<button ref={composeButtonRef} className="button button-green" type="button" onClick={() => setComposeOpen(true)}><PaperPlaneTilt aria-hidden="true" /> Nouveau message</button>} />
