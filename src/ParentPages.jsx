...
export function ParentLoginPage() {
  const account = DEMO_ACCOUNTS.parent; const { signIn } = useDemoStore(); const [identifier, setIdentifier] = useState(account.identifier); const [password, setPassword] = useState(account.password); const [message, setMessage] = useState("");
  ...
  return <ParentLoginView identifier={identifier} password={password} setIdentifier={setIdentifier} setPassword={setPassword} message={message} submit={submit}/>;
}

// Preserve the validated two-panel parent presentation independently of demo state.
export function ParentLoginView({identifier,password,setIdentifier,setPassword,message,submit,busy=false,recipe=false,onRecovery}) {
  const [visible,setVisible]=useState(false),[accessInfo,setAccessInfo]=useState("");
  return <div className="parent-login-page"><a className="skip-link" href="#parent-login-main">Aller au formulaire</a><aside className="parent-login-aside"><a href={recipe?"/connexion":"/connexion?mode=demo"} className="parent-login-back"><ArrowLeft aria-hidden="true" /> Choix du profil</a><img className="parent-login-logo" src="/assets/jet-dencre-logo-horizontal-light-400.webp" alt="Jet d’Encre Éditions" width="200" height="73" /><div className="parent-login-intro"><span className="parent-login-picture"><img src="/assets/parent-login-illustration-w07.png" alt="Illustration en pâte modelée d’une enseignante devant son tableau" width="1408" height="768" /></span><h1>Espace<br />Parent</h1><p>Accompagnez votre enfant avec une vision claire, rassurante et respectueuse de son autonomie.</p></div><ul><li><CheckCircle weight="fill" aria-hidden="true" /> Devoirs et échéances réunis</li><li><CheckCircle weight="fill" aria-hidden="true" /> Progression expliquée simplement</li><li><CheckCircle weight="fill" aria-hidden="true" /> Brouillons de messages locaux</li></ul></aside><main id="parent-login-main" className="parent-login-main" tabIndex="-1"><section className="parent-login-card"><span className="parent-login-badge"><Users weight="fill" aria-hidden="true" /> ESPACE PARENT</span><h2>{recipe?"Bienvenue dans votre espace parent":"Bienvenue dans la démo parent"}</h2><p>{recipe?"Retrouvez les informations utiles pour accompagner votre enfant au quotidien.":"Retrouvez les informations utiles pour accompagner Lina au quotidien."}</p><form onSubmit={submit} aria-busy={busy}><label>Adresse e-mail du parent<input type="email" maxLength={254} disabled={busy} autoComplete={recipe?"off":"username"} value={identifier} onChange={(event) => setIdentifier(event.target.value)} required /></label><label>Mot de passe<span className="parent-password-field"><input type={visible ? "text" : "password"} maxLength={128} disabled={busy} autoComplete={recipe?"off":"current-password"} value={password} onChange={(event) => setPassword(event.target.value)} required /><button type="button" onClick={() => setVisible((current) => !current)} aria-label={visible ? "Masquer le mot de passe" : "Afficher le mot de passe"}>{visible ? <EyeSlash aria-hidden="true" /> : <Eye aria-hidden="true" />}</button></span></label><div className="parent-login-options"><label><input type="checkbox" defaultChecked={!recipe} disabled={recipe} /> Rester connecté</label>{recipe?<button type="button" className="text-action" onClick={onRecovery}>Mot de passe oublié ?</button>:<ParentLink to="/mot-de-passe-oublie?mode=demo">Mot de passe oublié ?</ParentLink>}</div>{recipe&&<p className="parent-form-status" role="note">Recette locale · comptes fictifs uniquement. Session limitée à trois heures, sans prolongation automatique.</p>}{message && <p className="form-error" role="alert"><WarningCircle weight="fill" aria-hidden="true" /> {message}</p>}<button className="button button-green button-wide" type="submit" disabled={busy}>{busy?"Connexion…":"Accéder à ma famille"} <ArrowRight aria-hidden="true" /></button></form><button type="button" className="button button-light button-wide" onClick={() => setAccessInfo(recipe?"Le compte parent et le lien avec son enfant doivent être préparés par Jet d’Encre. Aucun compte n’est créé depuis cet écran.":"Cette démonstration ne crée aucun compte parent réel. Le rattachement sécurisé sera disponible après la mise en place du service par l’établissement.")}>Créer un accès parent</button>{accessInfo && <p className="parent-form-status" role="status">{accessInfo}</p>}</section></main></div>;
}
