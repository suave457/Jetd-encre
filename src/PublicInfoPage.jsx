...
export function PublicInfoPage({ path, ui }) {
  ...
  const isGuide = path === "/guide-ecole";
  const isReading = path === "/decouvrir";
  return <div className="blog-page public-info-page">
    ...
    <main>
      ...
      <div className={`public-info-sections${isGuide ? " public-help-steps" : isReading ? " public-reading-sections" : " public-info-grid"}`}>{page.sections.map(([title, copy], i) => isGuide ?
        <details key={title} className="panel" open={i === 0}><summary>{title}</summary><p>{copy}</p></details> :
