...
function StudentReader({ ui }) {
  ...
  const [marked, setMarked] = useState(() => localStorage.getItem("jde.reader.bookmark") === "true");
  const [notes, setNotes] = useState(() => localStorage.getItem("jde.reader.notes") || "");
  ...
  const toggleBookmark = () => {
    const next = !marked;
    setMarked(next);
    localStorage.setItem("jde.reader.bookmark", String(next));
    setNotice(next ? "Leçon ajoutée à tes favoris." : "Leçon retirée de tes favoris.");
  };
  const saveNotes = () => {
    localStorage.setItem("jde.reader.notes", notes);
    setNotice("Tes notes sont enregistrées sur cet appareil.");
  };
  const toggleAudio = () => {
...
function TeacherFlow({ screen, params, ui }) {
  ...
  if (screen === "teacher.assignment-preview") {
    ...
    const publish = () => { updateAssignment(assignment.id, { status: "Publié" }); notify({ role: "eleve", title: `Devoir publié · ${assignment.title}`, message: "La classe 5A peut maintenant consulter le devoir.", action: `/eleve/devoirs/${assignment.id}` }); go(`/enseignant/devoirs/${assignment.id}/remises`); };
    return <><PageHeader eyebrow="PRÉVISUALISATION" title={assignment.title} subtitle="Classe 5A · 29 élèves · Remise texte ou audio" action={<button className="button button-dark" onClick={publish}><PaperPlaneTilt/> Publier le devoir</button>}/><div className="pen-preview-layout"><section className="panel pen-student-preview"><span className="status-pill warning">À FAIRE</span><h2>{assignment.title}</h2><p>{assignment.instructions}</p><div className="pen-audio-drop"><Headphones weight="duotone"/><strong>Répondre à la consigne</strong><span>Texte ou audio selon l’activité · transcription disponible</span></div></section><aside className="panel"><h2>Vérifications</h2>{["Consigne claire", "Classe sélectionnée", "Échéance définie", "Format accessible"].map(item => <p className="pen-check-line" key={item}><CheckCircle weight="fill"/>{item}</p>)}<RouteLink to="/enseignant/devoirs/nouveau" className="button button-light button-wide">Modifier</RouteLink></aside></div></>;
  }
  if (screen === "teacher.submissions") {
    if (!assignment) return <SystemState params={{ systemState: "page-introuvable" }} ui={ui}/>;
    const remind = () => { notify({ role: "eleve", title: `Rappel · ${assignment.title}`, message: "Pense à envoyer ta réponse avant l’échéance.", action: `/eleve/devoirs/${assignment.id}` }); setNotice("Un rappel a été ajouté aux notifications des élèves."); };
    return <><PageHeader eyebrow="SUIVI DES REMISES" title={assignment.title} subtitle={`${related.length} remise${related.length > 1 ? "s" : ""} reçue${related.length > 1 ? "s" : ""} · Classe 5A`}/>{notice && <div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Rappel préparé</strong><small>{notice}</small></span></div>}<div className="pen-submission-summary"><ProgressBar value={related.length ? Math.round((related.length / 29) * 100) : 0} label="Remises reçues"/><button className="button button-light" onClick={remind}><Megaphone/> Relancer les élèves</button></div><div className="list-panel">{related.length ? related.map(item => <article className="pen-submission-row" key={item.id}><span>{item.studentName.split(" ").map(part => part[0]).join("").slice(0,2)}</span><div><h3>{item.studentName}</h3><p>{item.status} · {item.mediaType === "audio" ? "remise audio" : "remise texte"}</p></div><span className={`status-pill ${item.status === "Corrigé" ? "success" : "warning"}`}>{item.status}</span><RouteLink to={`/enseignant/remises/${item.id}/correction-audio`} className="button button-light">Corriger <CaretRight/></RouteLink></article>) : <div className="empty-state"><ClipboardText/><h2>Aucune remise</h2><p>Les réponses des élèves apparaîtront ici.</p></div>}</div></>;
  }
  if (screen === "teacher.audio-correction") {
    ...
    return <><PageHeader eyebrow="CORRECTION DE LA REMISE" title={submission.studentName} subtitle={`${resolvedAssignment.title} · Classe 5A`}/>{notice && <div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Correction envoyée</strong><small>{notice}</small></span></div>}<div className="pen-audio-correction"><section className="panel"><h2>{submission.mediaType === "audio" ? "Écouter la remise" : "Lire la remise"}</h2>{submission.mediaType === "audio" ? <><div className="pen-waveform"><button type="button" aria-label="Lire la remise audio"><PlayCircle weight="fill"/></button>{Array.from({ length: 34 }, (_, index) => <i style={{ height: `${18 + (index * 13) % 52}%` }} key={index}/>)}</div><p className="pen-transcript"><strong>Transcription automatique</strong> {submission.answer}</p></> : <blockquote className="pen-text-submission">{submission.answer}</blockquote>}</section><form className="panel" onSubmit={submit}><label>Note sur 20<input type="number" min="0" max="20" value={score} onChange={event => setScore(event.target.value)} required/></label><label>Commentaire formatif<textarea value={feedback} onChange={event => setFeedback(event.target.value)} required/></label><button className="button button-dark button-wide"><PaperPlaneTilt/> Envoyer la correction</button></form></div></>;
  }
  return null;
}
...
export function PublicContentPage({ params, ui }) {
  ...
  const content = useMemo(() => contents.find(item => (item.id.toLowerCase() === params.slug || String(item.slug || "") === params.slug) && item.status === "Publié"), [contents, params.slug]);
