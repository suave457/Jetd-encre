...
import { STUDENT_AVATARS } from "./features/student/studentProfileCore.js";
...
export function PenAccessPage({ screen, ui, connected = false }) {
  ...
  if (screen === "auth.forgot-password") {
    return <AuthLayout title="Retrouver mon accès" intro="Indiquez l’adresse associée à votre espace. Nous simulons ici l’envoi d’un lien sécurisé.">
      ...
      <form className="login-form" onSubmit={event => { event.preventDefault(); setDone(true); }}><label>Adresse e-mail<input type="email" defaultValue="demo@jetdencre.ma" autoComplete="email" required/></label><button className="button button-dark button-wide" type="submit"><PaperPlaneTilt/> Recevoir le lien</button><RouteLink to="/connexion?mode=demo" className="button button-light button-wide"><ArrowLeft/> Retour à la connexion</RouteLink></form>}
    </AuthLayout>;
  }
  ...
  if (["activation.invalid", "activation.used", "activation.expired", "activation.success"].includes(screen)) {
    const states = {
      ...
      "activation.success": [CheckCircle, connected ? "Ton accès est enregistré !" : "Ton manuel est prêt !", connected ? "Le manuel est maintenant associé à ton compte élève. Retrouve son état de disponibilité dans Mes manuels." : "Créons maintenant ton profil pour personnaliser ton parcours.", connected ? "/pilote?profil=eleve&section=manuels" : "/eleve/onboarding/profil", connected ? "Voir mes manuels" : "Créer mon profil"],
    };
    const [Icon, title, copy, to, label] = states[screen];
    return <AuthLayout title="Activation du manuel" intro="Un état clair pour savoir exactement quoi faire ensuite."><div className={`pen-activation-state ${screen === "activation.success" ? "success" : ""}`} role="status"><span><Icon weight="duotone"/></span><h2>{title}</h2><p>{copy}</p><RouteLink to={!connected&&to.startsWith('/activation')?to+'?mode=demo':to} className="button button-dark">{label} <ArrowRight/></RouteLink>{screen !== "activation.success" && <RouteLink to="/" className="button button-light">Retour à l’accueil</RouteLink>}</div></AuthLayout>;
  }
  return null;
}
...
const PROFILE_AVATAR_ICONS = { sparkle: Sparkle, book: BookOpenText, trophy: Trophy, pencil: NotePencil };
export function StudentProfile({ ui, user=null, classes=[], profile=null, avatarBusy=false, profileError="", onAvatarSave, onProfileRetry }) {
  const { PageHeader, RouteLink } = ui;
  const [active, setActive] = useState("profil");
  const [avatarOpen,setAvatarOpen]=useState(false);
  const initials=user?user.name.split(/\s+/).map(word=>word[0]).join("").slice(0,2):"LM";
  const Avatar=PROFILE_AVATAR_ICONS[profile?.avatar];
  async function chooseAvatar(value){setNotice("");if(await onAvatarSave(value)){setAvatarOpen(false);setNotice("Ton avatar est enregistré sur ton compte.");}}
  ...
  const requestAdult = () => setNotice(user?"Aucun message automatique n’a été envoyé. Préviens directement un parent, ton enseignant ou un adulte de confiance.":"Démonstration : aucun message n’a été envoyé. Si tu as besoin d’aide, préviens directement un parent, ton enseignant ou un adulte de confiance.");
  return <><PageHeader eyebrow="MON COMPTE" title="Profil & aide" subtitle="Personnalise ton espace et retrouve de l’aide quand tu en as besoin."/>{notice && <div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Information</strong><small>{notice}</small></span></div>}<div className="pen-profile-layout"><nav aria-label="Profil et aide">{[["profil",UserCircle,"Mon profil"],["accessibilite",Sparkle,"Confort de lecture"],["aide",ShieldCheck,"Aide et sécurité"]].map(([key,Icon,label]) => <button type="button" aria-current={active === key ? "page" : undefined} className={active === key ? "active" : ""} onClick={() => setActive(key)} key={key}><Icon/><span>{label}</span><CaretRight/></button>)}</nav><section className="panel"><h2>{active === "profil" ? "Mon profil" : active === "accessibilite" ? "Confort de lecture" : "Comment pouvons-nous t’aider ?"}</h2>{active === "profil" && <div className="pen-profile-card"><span aria-label={profile?STUDENT_AVATARS.find(item=>item.id===profile.avatar)?.label:undefined}>{Avatar?<Avatar weight="duotone" aria-hidden="true"/>:initials}</span><div><strong>{user?user.name:"Lina Mansouri"}</strong><small>{user?[classes.map(group=>group.name).join(", "),user.schoolName].filter(Boolean).join(" · "):"5A · École Al Manar"}</small></div><button className="button button-light" type="button" disabled={Boolean(user)&&(avatarBusy||!profile)} aria-expanded={user?avatarOpen:undefined} onClick={() => user?setAvatarOpen(value=>!value):setNotice("Choisis un nouvel avatar lors de ton prochain passage dans l’onboarding.")}>{avatarBusy?"Enregistrement…":"Modifier l’avatar"}</button></div>}{active==="profil"&&user&&<>{profileError?<p className="form-error" role="alert">{profileError} <button className="button button-light" type="button" onClick={onProfileRetry} disabled={avatarBusy}>Réessayer</button></p>:!profile&&<p role="status">Chargement de ton avatar…</p>}{avatarOpen&&profile&&<fieldset className="pen-avatar-choice"><legend>Choisis ton avatar</legend>{STUDENT_AVATARS.map(item=>{const ChoiceIcon=PROFILE_AVATAR_ICONS[item.id];return <button key={item.id} type="button" className={profile.avatar===item.id?"active":""} aria-label={item.label} aria-pressed={profile.avatar===item.id} disabled={avatarBusy} onClick={()=>chooseAvatar(item.id)}>{ChoiceIcon?<ChoiceIcon weight="duotone" aria-hidden="true"/>:initials}</button>;})}</fieldset>}</>}{active === "accessibilite" && <div className="pen-settings-list"><label>Taille du texte<select value={textSize} onChange={event => setTextSize(event.target.value)}><option>Standard</option><option>Confortable</option><option>Grand</option></select></label><label><span>Limiter les animations</span><input type="checkbox" checked={reduceMotion} onChange={event => setReduceMotion(event.target.checked)}/></label><label><span>Transcription automatique</span><input type="checkbox" checked={autoTranscript} onChange={event => setAutoTranscript(event.target.checked)}/></label><p className="pen-setting-preview">Aperçu : le français devient plus confortable à lire.</p></div>}{active === "aide" && <div className="pen-help-grid"><RouteLink to={user?"/guide-ecole":"/eleve/etat-systeme/hors-connexion"}><WarningCircle/><strong>Un problème de connexion</strong><span>Vérifier et réessayer</span></RouteLink><button type="button" onClick={requestAdult}><Megaphone/><strong>Contacter un adulte</strong><span>Savoir comment demander de l’aide</span></button></div>}</section></div></>;
}

// Original manual-card composition, with no demo activation, unit or progress injected.
export function StudentSchoolManuals({ui,manuals=[]}) {
  const {PageHeader,ResponsiveImage,RouteLink}=ui;
  return <><PageHeader eyebrow="MES MANUELS" title="Choisis ton manuel" subtitle="Retrouve ici les manuels associés à ton compte." action={<RouteLink to="/activation" className="button button-light"><Plus/> Activer un manuel</RouteLink>}/><div className="pen-manual-grid">
{manuals.map(manual=><article key={manual.id} className="pen-manual-card featured">{manual.level==='Document de test privé'?<section className="mediatheque-local-cover" aria-label="Document de test privé, sans couverture Jet d’Encre"><BookOpenText weight="duotone"/><span>Document de test privé</span></section>:<ResponsiveImage fileName="generated-1774018865796.png" alt="Illustration de la collection Jet d’Encre" sizes="(max-width: 900px) 100vw, 50vw"/>}<div><span className="status-pill success">ACCÈS ENREGISTRÉ</span><h2>{manual.title}</h2><p>{manual.level}</p><p>Cet accès est enregistré. Les fichiers disponibles se trouvent dans tes documents privés. Aucune progression scolaire n’est déduite de la lecture.</p></div></article>)}
    <article className="pen-manual-card locked"><BookOpenText weight="duotone"/><h2>{manuals.length?'Un autre manuel ?':'Ton premier manuel'}</h2><p>{manuals.length?'Active le code imprimé dans ton livre pour l’ajouter ici.':'Aucun manuel n’est encore associé à ton compte. Le code fourni par ton établissement permettra de l’ajouter ici.'}</p><RouteLink to="/activation" className="button button-light"><Key/> Saisir un code</RouteLink></article>
  </div></>;
}
...
export function PublicContentPage({ params, ui }) {
  ...
  return <div className="pen-public-content"><header><RouteLink to="/"><img src="/assets/jet-dencre-logo-horizontal-light-400.webp" alt="Jet d’Encre Éditions"/></RouteLink><nav><RouteLink to="/">Accueil</RouteLink><RouteLink to="/blog">Blog</RouteLink><RouteLink to="/guide-ecole">Guide d’accueil</RouteLink><RouteLink to="/pilote">Connexion</RouteLink></nav></header><main><section><div><span className="eyebrow">{content.type} · JET D’ENCRE</span><h1>{content.title}</h1><p>Une ressource éditoriale conçue pour écouter, comprendre et agir dans un contexte marocain plurilingue.</p><button type="button" className="button button-gold" onClick={start}><PlayCircle weight="fill"/> {started ? "Arrêter" : "Commencer"}</button>{notice && <p role="status">{notice}</p>}</div><ResponsiveImage fileName="generated-1774007681359.png" alt="Illustration de la ressource audio" eager sizes="(max-width: 900px) 100vw, 48vw"/></section><article><h2>À propos de cette ressource</h2><p>Écoutez les voix du quartier, repérez les lieux évoqués puis préparez une courte description de votre propre environnement. La lecture vocale sert d’aperçu accessible tant que le média définitif n’est pas publié.</p><div className="pen-public-facts"><span><strong>{content.level || "5e AEP"}</strong>Niveau</span><span><strong>{content.type}</strong>Format</span><span><strong>Avec transcription</strong>Accessibilité</span></div></article></main></div>;
}
