...
function AppShell({role,page,detail,screen,params,studentXp=DEFAULT_STUDENT_XP}){
  ...
  return <div className={`app-shell role-shell-${role}`}>
    ...
    <div className="app-area" inert={compact&&sidebarOpen?true:undefined}>
      <header className="app-topbar"><button ref={menuButton} className="sidebar-toggle" onClick={openSidebar} aria-label="Ouvrir le menu" aria-controls="workspace-navigation" aria-expanded={sidebarOpen}><List/></button>{searchEnabled?<div className="app-search"><MagnifyingGlass/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={role==="parent"?"Rechercher un enfant, un devoir, un message…":`Rechercher dans ${pageTitle.toLowerCase()}`} aria-label={role==="parent"?"Rechercher dans l’espace famille":`Rechercher dans ${pageTitle.toLowerCase()}`}/></div>:<div className="app-search-placeholder" aria-hidden="true"/>}<div className="topbar-meta">{role==="eleve"&&<div className="student-xp-chip" aria-live="polite" aria-label={`${studentXp} points d’expérience`}><Sparkle weight="fill"/><strong>{studentXp} XP</strong></div>}{role==="parent"?<span className="parent-connected"><i/> Famille connectée</span>:<><DemoBadge/><span>Année scolaire 2026–2027</span></>}<button className="notification" onClick={()=>setPanel(panel==="notifications"?null:"notifications")} aria-label={`${unreadNotifications.length} notification${unreadNotifications.length===1?"":"s"} non lue${unreadNotifications.length===1?"":"s"}`} aria-expanded={panel==="notifications"}><Bell/>{unreadNotifications.length>0&&<i>{unreadNotifications.length}</i>}</button><button className="profile-button" onClick={()=>setPanel(panel==="profile"?null:"profile")} aria-label={`Ouvrir le profil de ${currentUser?.name||config.name}`} aria-expanded={panel==="profile"}><span>{config.initials}</span><span><strong>{currentUser?.name||config.name}</strong><small>{config.label}</small></span><CaretDown/></button></div></header>
