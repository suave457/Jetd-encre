...
const roleConfigs = {
  eleve: {
    ...
    nav: [["tableau-de-bord","Accueil",House],["manuels","Mes manuels",BookOpenText],["devoirs","Mes devoirs",ClipboardText],["mediatheque","Audios & vidéos",PlayCircle],["jeux","Mes jeux",GameController],["progression","Mes progrès",ChartLineUp],["recompenses","Récompenses",Trophy],["profil","Profil & aide",UserCircle]],
  },
  parent: {
...
const blogArticles = [
  ...
  {
    slug:"plurilinguisme-force-classe", category:"Enseignants", theme:"Plurilinguisme", audience:"Professionnels", date:"21 août 2026", readTime:"8 min", author:"Dr Samira Berrada",
...
function usePublicArticles(){
  ...
  return useMemo(()=>{
    ...
    const untouched=blogArticles.filter(item=>!storedSlugs.has(item.slug));
    const published=articles.filter(item=>item.status==="Publié").map((item,index)=>{
      const source=blogArticles.find(article=>article.slug===item.slug);
      ...
      return {
        ...
        sections:source?.sections||(paragraphs.length?paragraphs.slice(0,3).map((copy,part)=>[`Partie ${part+1}`,copy]):[["À retenir",item.excerpt||"Cet article sera enrichi par l’équipe éditoriale."]]),
      };
    });
...
function useRoute() {
  ...
  useEffect(()=>{
    ...
    const routeLabel=route.split("/").filter(Boolean).at(-1)?.replaceAll("-"," ")||"Accueil";
    document.title=`${routeLabel.charAt(0).toUpperCase()+routeLabel.slice(1)} · Jet d’Encre`;
...
function LandingPage() {
  const [faq,setFaq]=useState(0); const [menuOpen,setMenuOpen]=useState(false);
  const faqs = [
    ["À quel âge mon enfant peut-il commencer ?","La collection accompagne les élèves du primaire, du niveau 1 au niveau 6. Un test d’orientation aide chaque enfant à démarrer au bon rythme."],
    ["Le contenu est-il conforme au programme marocain ?","Oui. Les parcours sont conçus pour le contexte scolaire marocain, avec une progression FLE structurée, des situations locales et des compétences communicatives."],
    ["Comment accéder au contenu numérique ?","Saisissez le code unique présent dans le manuel. L’espace personnel s’ouvre ensuite avec les audios, vidéos, activités et ressources du niveau choisi."],
    ["Puis-je utiliser les manuels en classe ?","Oui. L’espace enseignant permet de projeter les ressources, préparer des devoirs, suivre les progrès et différencier les activités."],
  ];
  return <div className="landing-page">
    <header className="public-header">
      <RouteLink to="/" className="public-logo" aria-label="Jet d’Encre — accueil"><ResponsiveImage fileName="jet-dencre-logo-horizontal-light.png" alt="Jet d’Encre Éditions" eager sizes="198px" /></RouteLink>
      <button className="menu-button" onClick={()=>setMenuOpen(!menuOpen)} aria-label="Ouvrir le menu" aria-expanded={menuOpen}>{menuOpen?<X/>:<List/>}</button>
      <nav className={menuOpen?"public-nav is-open":"public-nav"} aria-label="Navigation principale">
        <button onClick={()=>{scrollToId("collection");setMenuOpen(false)}}>Nos manuels</button><button onClick={()=>{scrollToId("univers");setMenuOpen(false)}}>Ressources</button><button onClick={()=>{scrollToId("methode");setMenuOpen(false)}}>Notre approche</button><RouteLink to="/blog" onClick={()=>setMenuOpen(false)}>Blog éducation</RouteLink><button onClick={()=>{scrollToId("faq");setMenuOpen(false)}}>Aide</button>
      </nav>
    ...
    <main>
      <section className="hero zellige-section">
        <div className="hero-copy"><span className="eyebrow"><Sparkle weight="fill"/> La référence francophone au Maroc</span><h1>Parlons bien,<br/>parlons <em>français&nbsp;!</em></h1><p>Une méthode de français joyeuse et ancrée dans la culture marocaine, conçue pour faire parler, lire et grandir chaque enfant avec confiance.</p><div className="hero-actions"><RouteLink to="/activation" className="button button-gold">Activer mon code manuel <ArrowRight weight="bold"/></RouteLink><button className="button button-light" onClick={()=>scrollToId("methode")}>Découvrir la méthode <PlayCircle weight="fill"/></button></div><div className="trust-points"><span><CheckCircle weight="fill"/> Programme marocain</span><span><CheckCircle weight="fill"/> Activités ludiques</span><span><CheckCircle weight="fill"/> Suivi des progrès</span></div></div>
        <div className="hero-visual"><div className="hero-image-frame"><ResponsiveImage fileName="generated-1773971894915.png" alt="Manuel illustré ouvert sur la kasbah et la cérémonie du thé" eager sizes="(max-width: 900px) 90vw, 590px"/></div><span className="floating-pill pill-a"><Headphones weight="fill"/> Audios inclus</span><span className="floating-pill pill-b"><GameController weight="fill"/> Jeux interactifs</span><span className="floating-pill pill-c"><Medal weight="fill"/> Progrès valorisés</span><span className="hero-score">250<small>activités</small></span></div>
      </section>
      <section className="metrics" aria-label="Chiffres clés">{[[Buildings,"150+","écoles partenaires"],[Student,"45 000+","élèves actifs"],[Medal,"98 %","de satisfaction"],[Books,"6","niveaux disponibles"]].map(([Icon,value,label])=><article key={label}><Icon weight="duotone"/><strong>{value}</strong><span>{label}</span></article>)}</section>
      ...
      <section id="collection" className="section collection-section"><SectionHeading eyebrow="La collection complète" title={<>6 niveaux pour accompagner<br/><em>chaque étape de l’apprentissage</em></>} copy="Une progression pédagogique fluide, du premier mot à l’expression autonome."/><div className="levels-grid">{levelCards.map(([level,grade,title,badge,color],index)=><article className="level-card" key={level} style={{"--level-color":color}}><div className="level-cover"><span className="level-index">{index+1}</span><span className="level-shape"><BookOpenText weight="duotone"/></span><strong>{level}</strong><small>Le français facile</small></div><div className="level-info"><span>{grade}</span><h3>{title}</h3><small>{badge}</small></div></article>)}</div></section>
      ...
      <section className="proof-section"><SectionHeading light eyebrow="Ce qu’ils en disent" title="Ils nous font confiance"/><div className="quotes-grid"><blockquote>« Mes élèves osent enfin parler français. Les situations leur ressemblent et les activités numériques maintiennent leur attention. »<footer>Fatima · Enseignante, Casablanca</footer></blockquote><blockquote>« Je peux voir les progrès de ma fille sans la surveiller. Elle ouvre la plateforme d’elle-même pour continuer sa série. »<footer>Youssef · Parent, Rabat</footer></blockquote><blockquote>« La direction dispose d’une vision claire de l’adoption et les enseignants partagent désormais les mêmes repères. »<footer>Sara · Directrice, Marrakech</footer></blockquote></div><div className="final-cta"><div><span className="eyebrow"><Sparkle weight="fill"/> Activation instantanée</span><h2>Débloquez l’univers Jet d’Encre</h2><p>Votre code manuel donne accès à toutes les ressources de votre niveau.</p></div><RouteLink to="/activation" className="button button-gold">Activer mon code <ArrowRight weight="bold"/></RouteLink></div></section>
    </main>
    <footer className="public-footer"><Brand inverse/><div><strong>Découvrir</strong><button onClick={()=>scrollToId("collection")}>Nos manuels</button><button onClick={()=>scrollToId("univers")}>Ressources numériques</button><RouteLink to="/blog">Blog éducation</RouteLink><RouteLink to="/connexion">Se connecter</RouteLink></div><div><strong>Établissements</strong><RouteLink to="/connexion/enseignant">Espace enseignant</RouteLink><RouteLink to="/connexion/directeur">Espace direction</RouteLink><RouteLink to="/connexion/admin">Administration</RouteLink></div><div><strong>Informations</strong><RouteLink to="/mentions-legales">Mentions légales</RouteLink><RouteLink to="/confidentialite">Confidentialité</RouteLink><RouteLink to="/conditions-utilisation">Conditions d’utilisation</RouteLink><RouteLink to="/accessibilite">Accessibilité</RouteLink></div><p className="copyright">© 2026 Jet d’Encre Éditions · Prototype de démonstration · contact@jetdencre.ma · Casablanca, Maroc</p></footer>
  </div>;
}
...
function PublicSubHeader({active="blog"}){
  ...
  return <header className="public-header blog-public-header"><RouteLink to="/" className="public-logo" aria-label="Jet d’Encre — accueil"><ResponsiveImage fileName="jet-dencre-logo-horizontal-light.png" alt="Jet d’Encre Éditions" eager sizes="198px"/></RouteLink><button className="menu-button" onClick={()=>setMenuOpen(!menuOpen)} aria-label="Ouvrir le menu" aria-expanded={menuOpen}>{menuOpen?<X/>:<List/>}</button><nav className={menuOpen?"public-nav is-open":"public-nav"} aria-label="Navigation principale"><RouteLink to="/">Accueil</RouteLink><RouteLink to="/blog" className={active==="blog"?"active":""}>Blog éducation</RouteLink><RouteLink to="/activation">Activer un manuel</RouteLink><RouteLink to="/connexion">Espaces utilisateurs</RouteLink></nav><RouteLink to="/connexion" className="button button-dark header-login"><UserCircle weight="bold"/> Connexion</RouteLink></header>
}

function BlogCard({article,featured=false,compact=false}){
  const fileName=article.image.split("/").at(-1);
  return <article className={`blog-card ${featured?"featured":""} ${compact?"compact":""}`}><RouteLink to={`/blog/${article.slug}`} className="blog-card-image"><ResponsiveImage fileName={fileName} alt={article.imageAlt} sizes={featured?"(max-width: 900px) 100vw, 55vw":"(max-width: 680px) 100vw, 420px"}/><span>{article.category}</span></RouteLink><div className="blog-card-body"><div className="blog-meta"><span>{article.theme}</span><span><Clock/> {article.readTime}</span></div><h3><RouteLink to={`/blog/${article.slug}`}>{article.title}</RouteLink></h3><p>{article.excerpt}</p><RouteLink to={`/blog/${article.slug}`} className="read-link">Lire l’article <ArrowRight weight="bold"/></RouteLink></div></article>
}

function BlogPage(){
  const publicArticles=usePublicArticles();
  const [query,setQuery]=useState(""); const [category,setCategory]=useState("Tous"); const [subscribed,setSubscribed]=useState(false);
  ...
  const featured=publicArticles.find(article=>article.featured)||publicArticles[0];
  return <div className="blog-page"><PublicSubHeader/><main><section className="blog-hero zellige-section"><div><span className="eyebrow"><BookOpenText weight="fill"/> Le Mag Jet d’Encre</span><h1>Des ressources qui rapprochent<br/><em>l’école et la maison</em></h1><p>Conseils pratiques, culture marocaine et pédagogie active pour donner à chaque enfant l’envie de parler français.</p></div><div className="blog-hero-stats"><strong>2×</strong><span>par mois</span><small>Une veille utile, sans surcharge.</small></div></section><section className="blog-content"><div className="blog-toolbar"><label className="blog-search"><MagnifyingGlass/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Rechercher un article, un thème…" aria-label="Rechercher dans le blog"/></label><div className="blog-categories" aria-label="Filtrer par public">{["Tous","Parents","Enseignants","Enfants"].map(item=><button key={item} className={category===item?"active":""} onClick={()=>setCategory(item)} aria-pressed={category===item}>{item}</button>)}</div></div>{defaultView&&featured&&<section className="blog-featured"><div className="blog-section-title"><div><span className="eyebrow">À la une</span><h2>Le conseil de la semaine</h2></div><span>Mis à jour le {featured.date}</span></div><BlogCard article={featured} featured/></section>}<section className="blog-latest"><div className="blog-section-title"><div><span className="eyebrow">{defaultView?"À lire aussi":"Résultats"}</span><h2>{defaultView?"Nos derniers articles":`${filtered.length} article${filtered.length>1?"s":""}`}</h2></div>{!defaultView&&<button onClick={()=>{setQuery("");setCategory("Tous")}}>Réinitialiser les filtres</button>}</div>{filtered.length?<div className="blog-grid">{(defaultView?filtered.filter(x=>x.slug!==featured?.slug):filtered).map(article=><BlogCard article={article} key={article.slug}/>)}</div>:<div className="blog-empty"><MagnifyingGlass/><h3>Aucun article ne correspond</h3><p>Essayez un mot plus général ou choisissez une autre catégorie.</p><button className="button button-light" onClick={()=>{setQuery("");setCategory("Tous")}}>Voir tous les articles</button></div>}</section></section><section className="blog-newsletter"><div><span className="eyebrow"><PaperPlaneTilt weight="fill"/> La lettre Jet d’Encre</span><h2>Une idée utile, deux fois par mois</h2><p>Activités prêtes à tester, conseils FLE et découvertes éditoriales.</p></div>{subscribed?<div className="newsletter-success"><CheckCircle weight="fill"/><span><strong>Inscription confirmée</strong><small>Merci ! La prochaine lettre arrivera bientôt.</small></span></div>:<form onSubmit={e=>{e.preventDefault();setSubscribed(true)}}><label className="sr-only" htmlFor="newsletter-email">Adresse e-mail</label><input id="newsletter-email" type="email" placeholder="Votre adresse e-mail" required/><button className="button button-gold" type="submit">Je m’inscris <ArrowRight/></button></form>}</section></main><BlogFooter/></div>
}
...
function LoginPage({role}){
  ...
  return <AuthLayout title={`Connexion ${config.label.toLowerCase()}`} intro={`Accédez à votre ${config.space.toLowerCase()} avec les identifiants préremplis de la démonstration.`}><form className="login-form" onSubmit={submit}><label>{role==="eleve"?"Identifiant":"Adresse e-mail"}<input type={role==="eleve"?"text":"email"} autoComplete="username" value={identifier} onChange={e=>setIdentifier(e.target.value)} required/></label><label>Mot de passe<input type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} required/></label><div className="form-options"><label className="check-label"><input type="checkbox" defaultChecked/> Se souvenir de moi</label><RouteLink to="/mot-de-passe-oublie">Mot de passe oublié ?</RouteLink></div><div className="demo-credentials"><strong>Compte de démonstration</strong><span>{account.identifier}</span><span>Mot de passe : {account.password}</span></div>{message&&<p className="form-notice" role="status" aria-live="polite"><WarningCircle weight="fill"/> {message}</p>}<button className="button button-dark button-wide" type="submit">Ouvrir mon espace <ArrowRight weight="bold"/></button></form><RouteLink to="/connexion" className="back-choice"><ArrowLeft/> Choisir un autre profil</RouteLink></AuthLayout>
}
...
function ActivationPage(){
  ...
  const submit=(event)=>{event.preventDefault();const outcome=activateCode(code);if(outcome.ok){navigateRoute("/activation/succes");return;}navigateRoute(outcome.status==="already_used"?"/activation/acces-deja-active":"/activation/code-invalide");};
...
function AppShell({role,page,detail,screen,params,studentXp=DEFAULT_STUDENT_XP}){
  ...
  const closeSidebar=(restoreFocus=true)=>{setSidebarOpen(false);if(restoreFocus)requestAnimationFrame(()=>menuButton.current?.focus())};
  ...
  useEffect(()=>{if(sidebarOpen&&compact)requestAnimationFrame(()=>closeButton.current?.focus())},[sidebarOpen,compact]);
  const logout=()=>{signOut();navigateRoute("/connexion",{replace:true})};
  ...
  return <div className={`app-shell role-shell-${role}`}>
    ...
    <aside id="workspace-navigation" className={sidebarOpen?"app-sidebar is-open":"app-sidebar"} aria-label={`Menu ${config.label}`} aria-hidden={compact&&!sidebarOpen} inert={compact&&!sidebarOpen?true:undefined}>
      ...
      {role==="parent"&&<div className="parent-family-summary"><span>YM</span><span><strong>Famille Mansouri</strong><small>2 enfants accompagnés</small></span></div>}
...
function ProgressBar({value,label}){const safe=Math.max(0,Math.min(100,Number(value)||0));return <div className="progress-wrap"><div><span>{label}</span><strong>{safe}%</strong></div><div className="progress-track" role="progressbar" aria-label={label} aria-valuemin="0" aria-valuemax="100" aria-valuenow={safe}><i style={{width:`${safe}%`}}/></div></div>}
...
function DataTable({headers,rows,label="Tableau de données"}){return <div className="data-table"><table><caption className="sr-only">{label}</caption><thead><tr>{headers.map((header,index)=><th scope="col" key={`${String(header)}-${index}`}>{header}</th>)}</tr></thead><tbody>{rows.map((row,index)=><tr key={index}>{row.map((cell,cellIndex)=><td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div>}
...
function StudentAssignmentDetail({assignment}){
  ...
  const submit=event=>{event.preventDefault();setError("");const result=submitAssignment({assignmentId:assignment.id,answer});if(!result.ok){setError(result.message);return}setFeedback("Ta remise a bien été envoyée à Mme Benjelloun.");navigateRoute(`/eleve/devoirs/${assignment.id}/remise-confirmee`)};
  return <><nav className="detail-breadcrumb" aria-label="Fil d’Ariane"><RouteLink to="/eleve/devoirs"><ArrowLeft/> Retour à mes devoirs</RouteLink></nav><PageHeader eyebrow="MON DEVOIR" title={assignment.title} subtitle={`${assignment.subject||"Français"} · à rendre le ${new Date(assignment.dueAt).toLocaleDateString("fr-FR")}`} serif action={<span className={`status-pill ${reviewed?"success":"warning"}`}>{reviewed?`Corrigé · ${existing.score}/20`:submitted?"Remis · correction en attente":"À faire"}</span>}/>{feedback&&<div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Remise enregistrée</strong><small>{feedback}</small></span></div>}{reviewed&&<div className="assignment-feedback" role="status"><Trophy weight="duotone"/><div><span>CORRECTION DE MME BENJELLOUN</span><h2>{existing.score}/20</h2><p>{existing.feedback}</p></div></div>}<div className="assignment-detail-layout"><form className="panel assignment-response" onSubmit={submit}><span className="eyebrow">Consigne</span><h2>{assignment.instructions}</h2><label htmlFor="student-assignment-answer">Ma réponse</label><textarea id="student-assignment-answer" value={answer} onChange={event=>setAnswer(event.target.value)} placeholder="Écris ta réponse ici…" readOnly={reviewed} aria-describedby="student-answer-help student-answer-error"/><small id="student-answer-help">Minimum 12 caractères · Ta réponse reste enregistrée sur cet appareil.</small>{error&&<p className="form-error" id="student-answer-error" role="alert"><WarningCircle/>{error}</p>}<button className="button button-dark" type="submit" disabled={reviewed}><PaperPlaneTilt/>{submitted?"Mettre à jour ma remise":"Envoyer ma remise"}</button></form><aside className="panel assignment-guide"><ClipboardText weight="duotone"/><h2>Avant d’envoyer</h2><ol><li>Relis la consigne.</li><li>Vérifie les cinq adjectifs.</li><li>Corrige les accords simples.</li></ol><div className="prototype-note"><DemoBadge/><p>Cette remise est fictive et stockée uniquement dans ce navigateur.</p></div></aside></div></>;
}
...
function TeacherAssignmentDetail({assignment}){
  ...
  const [score,setScore]=useState(selected?.score??16);
  const [comment,setComment]=useState(selected?.feedback||"Très bon travail. Pense à varier les connecteurs pour rendre ta description encore plus fluide.");
  const [notice,setNotice]=useState("");
  const [error,setError]=useState("");
  useEffect(()=>{if(!selected)return;setScore(selected.score??16);setComment(selected.feedback||"Très bon travail. Pense à varier les connecteurs pour rendre ta description encore plus fluide.");setNotice("");setError("")},[selected?.id]);
  const review=event=>{event.preventDefault();setError("");const result=reviewSubmission(selected.id,{score,feedback:comment});if(!result.ok){setError(result.message);return}setNotice(`La correction de ${selected.studentName} est envoyée.`)};
...
function RoleDetailPage({role,page,detail}){
  const {assignments,contents,updateContent,notify}=useDemoStore();
  const [feedback,setFeedback]=useState("");
  ...
  const assignment=page==="devoirs"?resolveRouteRecord(assignments,detail):null;
...
function StudentDashboard(){const {manualActivations,manualProgress,quizAttempts,assignments,submissions,session}=useDemoStore();const activations=manualActivations.filter(item=>item.userId===session.userId);const active=activations.at(-1);const progress=[...manualProgress].reverse().find(item=>item.userId===session.userId&&(!active||item.activationId===active.id))||manualProgress.find(item=>item.userId===session.userId);const percent=Number(progress?.percent||0);const latestQuiz=quizAttempts.find(item=>item.userId===session.userId);const submittedIds=new Set(submissions.filter(item=>item.studentId===session.userId).map(item=>item.assignmentId));const todo=assignments.filter(item=>item.status==="Publié"&&!submittedIds.has(item.id));return <><PageHeader eyebrow="LUNDI 24 AOÛT" title="Bonjour Lina" subtitle="Qu’est-ce que tu veux faire aujourd’hui ?" action={<RouteLink to="/eleve/manuels/francais-5/lecons/lecon-2" className="button button-gold">Continuer ma leçon <ArrowRight/></RouteLink>}/><section className="student-journey"><div><span>TON PARCOURS · {active?.level||"5e AEP"}</span><h2>Unité 3 · Protégeons notre environnement</h2><p>Tu as terminé {percent} % de ce parcours. {percent<100?"Chaque activité fait avancer ton manuel.":"Bravo, cette unité est terminée !"}</p><ProgressBar value={percent} label="Progression de l’unité"/></div><ResponsiveImage fileName="generated-1774018865796.png" alt="Livre illustré" sizes="230px"/></section><div className="dashboard-two-columns"><article className="panel assignment-panel"><div className="panel-top"><span className="status-pill warning">{todo.length} à faire</span><small>Prochaine échéance</small></div><h3>{todo[0]?.title||"Tous les devoirs sont remis"}</h3><p>{todo[0]?"Donné par Mme Salma Benjelloun · Environ 10 minutes":"Retrouve les corrections dans Mes devoirs."}</p>{todo[0]&&<ProgressBar value={0} label="À commencer"/>}</article><article className="streak-card"><small>DERNIÈRE ACTIVITÉ</small><strong>{latestQuiz?`${latestQuiz.scorePercent} %`:"À toi !"}</strong><b>{latestQuiz?"Quiz terminé":"Commence ton premier quiz"}</b><span>{latestQuiz?`${latestQuiz.correctCount}/${latestQuiz.questionCount} bonnes réponses · ${latestQuiz.xpEarned} XP`:"Culture générale du Maroc"}</span></article></div><div className="quick-grid"><RouteLink to="/eleve/manuels"><BookOpenText/><strong>Mes manuels</strong><span>{active?active.manualTitle:"Activer un manuel"}</span></RouteLink><RouteLink to="/eleve/devoirs"><ClipboardText/><strong>Mes devoirs</strong><span>{todo.length} à faire</span></RouteLink><RouteLink to="/eleve/jeux"><GameController/><strong>Mes jeux</strong><span>Quiz culture générale</span></RouteLink><RouteLink to="/eleve/progression"><ChartLineUp/><strong>Mes progrès</strong><span>{percent} % terminé</span></RouteLink></div></>}

function StudentManual(){const {manualActivations,manualProgress,session}=useDemoStore();const [lesson,setLesson]=useState(1);const lessons=["Observer et décrire","Écouter la ville","Lire une affiche","Préparer mon reportage"];const activations=manualActivations.filter(item=>item.userId===session.userId);const active=activations.at(-1);const progress=[...manualProgress].reverse().find(item=>item.userId===session.userId&&(!active||item.activationId===active.id));const percent=Math.max(Number(progress?.percent||0),(lesson+1)*25);return <><PageHeader eyebrow={`MON MANUEL · ${active?.level||"NIVEAU 5"}`} title="Voyage au Maroc" subtitle="Unité 3 · Ma ville, notre patrimoine" action={<button className="button button-light" disabled title="Téléchargement disponible après mise en production"><DownloadSimple/> Télécharger le mémo</button>}/>{active?<div className="manual-activation-strip" role="status"><span><Key weight="fill"/></span><div><strong>{active.manualTitle} est associé à ton profil</strong><small>Code {active.code} · {activations.length} manuel{activations.length>1?"s":""} activé{activations.length>1?"s":""}</small></div><span className="status-pill success">Activé</span></div>:<div className="wide-notice"><WarningCircle weight="fill"/><div><strong>Aucun manuel n’est encore associé à ce profil.</strong><span>Active le code présent dans ton livre pour enregistrer ta progression.</span></div><RouteLink to="/activation" className="button button-gold">Activer un manuel</RouteLink></div>}<div className="manual-layout"><section className="manual-viewer"><div className="manual-visual"><ResponsiveImage fileName="generated-1773971894915.png" alt="Double page du manuel Voyage au Maroc" sizes="(max-width: 900px) 100vw, 65vw"/></div><div className="manual-controls"><button disabled={lesson===0} onClick={()=>setLesson(Math.max(0,lesson-1))}><ArrowLeft/> Précédent</button><span>Leçon {lesson+1} sur {lessons.length}</span><button disabled={lesson===lessons.length-1} onClick={()=>setLesson(Math.min(lessons.length-1,lesson+1))}>Suivant <ArrowRight/></button></div></section><aside className="lesson-panel"><span className="status-pill success">En cours</span><h2>{lessons[lesson]}</h2><p>Découvre le vocabulaire de la ville et entraîne-toi à présenter un lieu de ton quartier.</p><button className="lesson-action" disabled title="Lecteur audio en attente des médias définitifs"><Headphones weight="fill"/><span><strong>Écouter le dialogue</strong><small>3 min · avec transcription</small></span><PlayCircle weight="fill"/></button><RouteLink to="/eleve/jeux/culture-generale" className="lesson-action"><GameController weight="fill"/><span><strong>Faire une activité</strong><small>10 questions · XP à gagner</small></span><ArrowRight/></RouteLink><ProgressBar value={percent} label="Unité 3"/></aside></div></>}

function StudentHomework(){
  const {assignments,submissions,session}=useDemoStore();
  const [tab,setTab]=useState("afaire");
  const [correction,setCorrection]=useState("");
  const studentSubmissions=submissions.filter(item=>item.studentId===(session.userId||"user-eleve-lina"));
  const submittedAssignmentIds=new Set(studentSubmissions.map(item=>item.assignmentId));
  const todo=assignments.filter(item=>item.status==="Publié"&&!submittedAssignmentIds.has(item.id));
  const completed=[...studentSubmissions.map(submission=>{const assignment=assignments.find(item=>item.id===submission.assignmentId);return [submission.id,assignment?.title||"Devoir remis",`Remis le ${new Date(submission.submittedAt).toLocaleDateString("fr-FR")}`,submission.status==="Corrigé"?`Corrigé · ${submission.score}/20`:"Remis · en attente",100,submission.status==="Corrigé"?submission.feedback:"Ta remise est enregistrée. Mme Benjelloun n’a pas encore envoyé sa correction."]}),["termine-quartier","Mon quartier idéal","Remis le 18 août","16 / 20",100,"Très bon travail : le lieu est bien organisé et les adjectifs sont variés."],["termine-medina","Compréhension orale · La médina","Remis le 12 août","8 / 10",100,"Bonne écoute. Pense à relire les deux mots de vocabulaire signalés."]];
  const items=tab==="afaire"?todo.map((item,index)=>[item.id,item.title,`${item.subject||"Français"} · pour le ${new Date(item.dueAt).toLocaleDateString("fr-FR")}`,index===0?"Commencé":"À faire",index===0?50:0]):completed;
  return <><PageHeader eyebrow="MON TRAVAIL" title="Mes devoirs" subtitle="Organise ton travail et retrouve les corrections de ton enseignante."/><Tabs value={tab} onChange={value=>{setTab(value);setCorrection("")}} items={[["afaire",`À faire · ${todo.length}`],["termines",`Terminés · ${completed.length}`]]}/>{correction&&<div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Retour de Mme Benjelloun</strong><small>{correction}</small></span></div>}<div className="list-panel">{items.map(([id,title,meta,status,value,feedback])=><article className="homework-row" key={id}><span className="row-icon"><ClipboardText weight="duotone"/></span><div><h3>{title}</h3><p>{meta}</p>{value>0&&value<100&&<ProgressBar value={value} label="Avancement"/>}</div><span className={value===100?"status-pill success":"status-pill warning"}>{status}</span>{tab==="afaire"?<RouteLink className="icon-button" to={`/eleve/devoirs/${id}`} aria-label={`Ouvrir ${title}`}><CaretRight/></RouteLink>:<button className="icon-button" aria-label={`Consulter le suivi de ${title}`} onClick={()=>setCorrection(feedback)}><Eye/></button>}</article>)}</div></>
}
...
function AdminBlogV2({search}){
  ...
  const save=(publish=false)=>{const nextStatus=publish?"Publié":form.status;const payload={...form,status:nextStatus,publishedAt:nextStatus==="Publié"?new Date().toISOString():null,scheduledAt:nextStatus==="Planifié"?`${form.scheduleDate}T08:00:00.000Z`:null};const result=editingId?updateArticle(editingId,payload):createArticle(payload);if(result.ok){setEditingId(result.item.id);setForm(current=>({...current,status:payload.status}));setNotice(nextStatus==="Publié"?"Article publié : il est maintenant visible dans le Blog public.":nextStatus==="Planifié"?`Article planifié pour le ${new Date(`${form.scheduleDate}T12:00:00`).toLocaleDateString("fr-FR")}.`:"Brouillon enregistré sur cet appareil.")}};
  if(mode==="editor")return <div className="admin-blog-editor"><PageHeader eyebrow="BLOG & ARTICLES" title={editingId?"Modifier l’article":"Nouvel article"} subtitle="Rédigez, ciblez et préparez la publication dans le prototype local." serif action={<button className="button button-light" onClick={()=>setMode("list")}><ArrowLeft/> Retour aux articles</button>}/>{notice&&<div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Enregistrement réussi</strong><small>{notice}</small></span></div>}<div className="blog-editor-layout"><section className="panel blog-editor-form"><h2>Contenu éditorial</h2><label>Titre de l’article<input value={form.title} onChange={event=>setForm({...form,title:event.target.value})} required/></label><label>Chapô<textarea value={form.excerpt} onChange={event=>setForm({...form,excerpt:event.target.value})} placeholder="Résumez la promesse de l’article en deux phrases."/></label><div className="form-two"><label>Catégorie<select value={form.category} onChange={event=>setForm({...form,category:event.target.value})}><option>Parents</option><option>Enseignants</option><option>Enfants</option></select></label><label>Thème<input value={form.theme} onChange={event=>setForm({...form,theme:event.target.value})}/></label></div><div className="form-two"><label>Auteur<input value={form.author} onChange={event=>setForm({...form,author:event.target.value})}/></label><label>Statut<select value={form.status} onChange={event=>setForm({...form,status:event.target.value})}><option>Brouillon</option><option>À réviser</option><option>Planifié</option><option>Publié</option></select></label></div>{form.status==="Planifié"&&<label>Date de publication<input type="date" min="2026-08-27" value={form.scheduleDate} onChange={event=>setForm({...form,scheduleDate:event.target.value})}/></label>}<label>Corps de l’article<textarea className="rich-textarea" value={form.body} onChange={event=>setForm({...form,body:event.target.value})} placeholder="Développez l’article avec des paragraphes clairs, des exemples marocains et une proposition immédiatement applicable."/></label></section><aside className="panel article-preview-panel"><div className="panel-heading"><h2>Aperçu</h2><DemoBadge/></div><article className="mini-article-preview text-preview"><span>{form.category} · {form.theme}</span><h3>{form.title||"Titre de votre article"}</h3><p>{form.excerpt||"Le chapô apparaîtra ici."}</p><small>{form.author} · lecture estimée 6 min</small></article><p className="prototype-copy">Une publication apparaît immédiatement dans le Blog public de cette démonstration locale.</p></aside></div><div className="blog-editor-actions"><button className="button button-light" disabled={!form.title.trim()} onClick={()=>save(false)}><PencilSimple/> Enregistrer</button><button className="button button-dark" disabled={!form.title.trim()||!form.excerpt.trim()||!form.body.trim()} onClick={()=>save(true)}><PaperPlaneTilt/> Publier l’article</button></div></div>;
  return <><PageHeader eyebrow="PUBLICATION ÉDITORIALE" title="Blog & articles" subtitle="Planifiez, relisez et publiez les contenus destinés aux familles et aux équipes pédagogiques." serif action={<button className="button button-dark" onClick={()=>openEditor()}><Plus/> Nouvel article</button>}/>{notice&&<div className="editor-success" role="status"><CheckCircle weight="fill"/><span><strong>Liste mise à jour</strong><small>{notice}</small></span></div>}<KpiGrid items={[["Articles publiés",String(articles.filter(item=>item.status==="Publié").length),"Visibles publiquement"],["Brouillons",String(articles.filter(item=>item.status==="Brouillon").length),"À compléter","gold"],["À réviser",String(articles.filter(item=>item.status==="À réviser").length),"File éditoriale"],["Total",String(articles.filter(item=>item.status!=="Archivé").length),"Hors archives"]]}/><div className="library-toolbar blog-admin-toolbar"><div className="tabs-inline" aria-label="Filtrer les articles">{["Tous","Publié","Brouillon","Planifié","À réviser","Archivé"].map(item=><button key={item} className={status===item?"active":""} aria-pressed={status===item} onClick={()=>setStatus(item)}>{item}</button>)}</div><RouteLink to="/blog" className="button button-light"><Eye/> Voir le Blog public</RouteLink></div><div className="admin-article-list store-articles">{filtered.map(article=><article key={article.id}><span className="article-placeholder"><FileText weight="duotone"/></span><div className="admin-article-title"><span>{article.category} · {article.theme}</span><strong>{article.title}</strong><small>{article.author} · {new Date(article.updatedAt).toLocaleDateString("fr-FR")}</small></div><span className={`status-pill ${article.status==="Publié"?"success":article.status==="Brouillon"?"neutral":"warning"}`}>{article.status}</span><span className="article-performance"><strong>{article.status==="Publié"?"Publié":"Non publié"}</strong><small>{article.status==="Planifié"&&article.scheduledAt?new Date(article.scheduledAt).toLocaleDateString("fr-FR"):"Prototype local"}</small></span><span className="row-actions">{article.status==="Publié"?<RouteLink to={`/blog/${article.slug}`} title="Aperçu public" aria-label={`Aperçu de ${article.title}`}><Eye/></RouteLink>:<button disabled title="Publiez l’article pour ouvrir son aperçu public" aria-label="Aperçu non disponible"><Eye/></button>}<button title="Modifier" aria-label={`Modifier ${article.title}`} onClick={()=>openEditor(article)}><PencilSimple/></button><button title="Dupliquer" aria-label={`Dupliquer ${article.title}`} onClick={()=>{duplicateArticle(article.id);setNotice("Une copie brouillon a été créée.")}}><Copy/></button><button title="Archiver" aria-label={`Archiver ${article.title}`} onClick={()=>{archiveArticle(article.id);setNotice(`« ${article.title} » a été archivé.`)}}><Archive/></button></span></article>)}</div>{filtered.length===0&&<div className="empty-state"><FileText/><h3>Aucun article dans cet état</h3><p>Choisissez un autre statut ou créez un nouvel article.</p></div>}</>
}
...
export function App(){
  ...
  if(parsed.kind==="landing")return <LandingPage/>;
