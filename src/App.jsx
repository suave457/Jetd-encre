...
function StudentDashboard() {
  ...
  const latestLabel=latest?.experienceType==="learning-activity"?"Activité corrigée":latest?.quizId==="mots-fleches"?"Grille terminée":latest?"Entraînement terminé":"Commence une activité";
  return <>
    <PageHeader eyebrow="MON ESPACE DE DÉMONSTRATION" title={`Bonjour ${currentUser?.name?.split(" ")[0]||"à toi"}`} subtitle="Lis, essaie, puis prends le temps de comprendre la correction." action={<RouteLink to="/eleve/activites/mots-environnement" className="button button-gold">M’entraîner <ArrowRight/></RouteLink>}/>
    <section className="student-journey"><div><span>JE PRATIQUE · {active?.level||"5e AEP"}</span><h2>Les mots de l’environnement</h2><p>Cinq questions pour comprendre et réutiliser le vocabulaire. Une explication t’aide après chaque réponse.</p><p>Les jeux et les XP ne valident pas automatiquement une unité du manuel.</p></div><ResponsiveImage fileName="generated-1774018865796.png" alt="Livre illustré" sizes="230px"/></section>
    <div className="dashboard-two-columns"><article className="panel assignment-panel"><div className="panel-top"><span className="status-pill warning">{todo.length} à faire</span><small>Prochaine échéance</small></div><h3>{todo[0]?.title||"Aucun devoir à remettre"}</h3><p>{todo[0]?`Pour le ${new Date(todo[0].dueAt).toLocaleDateString("fr-FR")}`:"Retrouve les retours disponibles dans Mes devoirs."}</p>{todo[0]&&<RouteLink to={`/eleve/devoirs/${todo[0].id}`} className="button button-light">Ouvrir le devoir <ArrowRight/></RouteLink>}</article><article className="streak-card"><small>DERNIÈRE ACTIVITÉ</small><strong>{latest?`${latest.correctCount}/${latest.questionCount}`:"À toi !"}</strong><b>{latestLabel}</b><span>{latest?`${latest.xpEarned||0} XP attribués · résultat de cette activité`:"Un résultat expliqué, à ton rythme"}</span></article></div>
    <div className="quick-grid"><RouteLink to="/eleve/manuels"><BookOpenText/><strong>Mes manuels</strong><span>{active?.manualTitle||"Découvrir le manuel"}</span></RouteLink><RouteLink to="/eleve/devoirs"><ClipboardText/><strong>Mes devoirs</strong><span>{todo.length} à faire</span></RouteLink><RouteLink to="/eleve/mediatheque"><Books/><strong>Médiathèque</strong><span>Audio, vidéo et livres</span></RouteLink><RouteLink to="/eleve/progression"><ChartLineUp/><strong>Mes résultats</strong><span>Activités et entraînements</span></RouteLink></div>
  </>;
}
...
function StudentHomework(){
  ...
  return <><PageHeader eyebrow="MON TRAVAIL · DÉMONSTRATION LOCALE" title="Mes devoirs" subtitle="Retrouve les devoirs de ta classe et les retours enregistrés sur cet appareil."/><Tabs value={tab} onChange={value=>{setTab(value);setCorrection("")}} items={[["afaire",`À faire · ${todo.length}`],["termines",`Remis · ${completed.length}`]]}/>{correction&&<div className="editor-success" role="status"><CheckCircle/><span><strong>Retour enregistré</strong><small>{correction}</small></span></div>}<div className="list-panel">{items.length?items.map(item=><article className="homework-row" key={item.id}><span className="row-icon"><ClipboardText/></span><div><h3>{item.title}</h3><p>{item.subject||"Français"} · pour le {new Date(item.dueAt).toLocaleDateString("fr-FR")}</p></div><span className={item.status==="Corrigé"?"status-pill success":"status-pill warning"}>{item.status}{item.status==="Corrigé"?` · ${item.submission.score}/20`:""}</span>{item.status==="À faire"?<RouteLink className="icon-button" to={`/eleve/devoirs/${item.id}`} aria-label={`Ouvrir ${item.title}`}><CaretRight/></RouteLink>:<button className="icon-button" aria-label={`Consulter le suivi de ${item.title}`} onClick={()=>setCorrection(item.status==="Corrigé"?item.submission.feedback:"La remise est enregistrée localement. Aucune correction n’est encore disponible.")}><Eye/></button>}</article>):<p className="popover-empty">Aucun devoir dans cette liste.</p>}</div></>;
}
