  ...
  return (
    ...
    >
      <header>
        ...
        <strong>{tier.completionPercent}%</strong>
      </header>
      <h3>{tier.label}</h3>
      <p>{tier.cardTitle}</p>
      <div className="tmd-tier-track" role="progressbar" aria-label={`Palier ${tier.label} terminé par ${tier.completionPercent} % de la cohorte`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={tier.completionPercent}>
        <i style={{ width: `${tier.completionPercent}%` }} />
      </div>
      <footer>
        <span><strong>{tier.completedStudents}</strong> sur {studentCount} ont terminé</span>
  ...
  return (
    <span className="tmd-mission-dots" aria-label={`${tier.completedCount} missions réussies sur ${tier.missionCount}`}>
      {tier.missionIds.map((missionId, index) => {
        ...
        const label = state === "is-autonomous" ? "réussie sans indice" : state === "is-guided" ? "réussie avec indice" : "à découvrir";
      ...
      <aside ref={drawer} className="tmd-drawer" role="dialog" aria-modal="true" aria-labelledby="tmd-student-title">
        <header className="tmd-drawer-header">
          ...
          <div>
            ...
            <small>{student.partialData ? "Donnée partielle issue d’un ancien bilan" : student.live ? "Résultats locaux mis à jour automatiquement" : "Profil fictif de démonstration"}</small>
          </div>
        ...
        <div className="tmd-drawer-body">
          <section className="tmd-student-kpis" aria-label="Synthèse de l’élève">
            ...
            <article><span>Aides</span><strong>{student.helpCount}</strong><small>{student.soukXp} XP au Souk</small></article>
          </section>
          ...
          <section className="tmd-drawer-section" aria-labelledby="tmd-tier-detail-title">
            ...
            <div className="tmd-dot-legend" aria-label="Légende des missions"><span><i className="is-autonomous" /> Réussie sans indice</span><span><i className="is-guided" /> Réussie avec indice</span><span><i /> À découvrir</span></div>
          </section>
          ...
          <section className={`tmd-diagnostic is-${student.status}`} aria-labelledby="tmd-diagnostic-title">
            ...
            <div>
              ...
              <p>Ce signal repose sur la progression, l’usage des aides et la date de la dernière activité.</p>
            </div>
          </section>
          ...
          <p className="tmd-data-note">
            Le prototype analyse uniquement des indicateurs de jeu. Il ne remplace pas l’observation de l’enseignant ni une évaluation en classe.
          </p>
        </div>
      </aside>
    </div>
...
export default function TeacherMarketDashboard({
  ...
  const dashboard = useMemo(() => buildTeacherMarketDashboard({ awardHistory, attemptHistory }), [awardHistory, attemptHistory]);
  ...
  const visibleStudents = useMemo(() => filterTeacherMarketStudents(dashboard.students, { classId, status, period, query }), [dashboard.students, classId, status, period, query]);
  const selectedStudent = dashboard.students.find((student) => student.id === selectedId) || null;
  const supportStudents = dashboard.students.filter((student) => student.status === "support").slice(0, 3);
  const csv = useMemo(() => buildTeacherMarketCsv(visibleStudents), [visibleStudents]);
  const exportHref = `data:text/csv;charset=utf-8,%EF%BB%BF${encodeURIComponent(csv)}`;
  const openStudent = (studentId, event) => { lastTrigger.current = event.currentTarget; setSelectedId(studentId); };
  ...
  return (
    <div className="teacher-market-dashboard">
      ...
      <header className="tmd-hero">
        <div className="tmd-hero-copy">
          ...
          <div className="tmd-hero-tags" aria-label="Périmètre du suivi">
            <span>5e AEP · Classes 5A et 5B</span>
            <span>12 missions</span>
            <span>Données locales de démonstration</span>
          </div>
        </div>
        <div className="tmd-hero-actions">
          <div><Sparkle weight="duotone" /><span><small>MISE À JOUR</small><strong>À chaque palier terminé</strong></span></div>
          <a className="tmd-export" href={exportHref} download="progression-souk-des-mots.csv"><DownloadSimple /> Exporter la vue</a>
        </div>
      </header>
      ...
      <section className="tmd-summary-grid" aria-label="Indicateurs du Souk des mots">
        ...
        <SummaryCard icon={CheckCircle} label="Missions maîtrisées" value={`${dashboard.summary.totalMasteries} / ${dashboard.summary.studentCount * dashboard.totalMissions}`} note={`${dashboard.summary.completedCount} parcours entièrement terminés`} tone="navy" />
        <SummaryCard icon={TrendUp} label="Réussies sans indice" value={`${dashboard.summary.totalAutonomousMasteries} / ${dashboard.summary.totalMasteries || 0}`} note={`${dashboard.summary.averageAutonomy}% des maîtrises documentées`} tone="gold" />
        <SummaryCard icon={WarningCircle} label="À accompagner" value={`${dashboard.summary.supportCount} / ${dashboard.summary.studentCount}`} note="Signal pédagogique prioritaire" tone="coral" />
      </section>
      ...
      <section className="tmd-tier-overview" aria-labelledby="tmd-tier-overview-title">
        <div className="tmd-section-heading">
          ...
          <small>{dashboard.summary.averageProgress}% de progression moyenne</small>
        </div>
      ...
      <div className="tmd-content-grid">
        <section className="tmd-roster" aria-labelledby="tmd-roster-title">
          ...
          <div className="tmd-filters" aria-label="Filtrer les élèves">
            ...
            <label><Funnel aria-hidden="true" /><span className="sr-only">Filtrer par classe</span><select value={classId} onChange={(event) => setClassId(event.target.value)}><option value="all">Toutes les classes</option><option value="classe-5a">Classe 5A</option><option value="classe-5b">Classe 5B</option></select></label>
            <label><Target aria-hidden="true" /><span className="sr-only">Filtrer par période</span><select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="all">Depuis le lancement</option><option value="30">30 derniers jours</option><option value="7">7 derniers jours</option></select></label>
            <label><TrendUp aria-hidden="true" /><span className="sr-only">Filtrer par statut</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Tous les statuts</option><option value="support">À accompagner</option><option value="on-track">En bonne voie</option><option value="completed">Parcours terminés</option><option value="not-started">À démarrer</option></select></label>
          </div>

          <div className="tmd-table-wrap">
            <table>
              <caption className="sr-only">Suivi de la progression des élèves dans Le Souk des mots</caption>
              <thead><tr><th scope="col">Élève</th><th scope="col">Palier</th><th scope="col">Missions</th><th scope="col">Sans indice</th><th scope="col">Aides</th><th scope="col">Statut</th><th scope="col"><span className="sr-only">Ouvrir</span></th></tr></thead>
              <tbody>
                {visibleStudents.map((student) => (
                  <tr key={student.id}>
                    ...
                    <td data-label="Aides"><span className={student.helpCount >= 2 ? "tmd-help-count is-high" : "tmd-help-count"}>{student.helpCount}</span></td>
