...
function TeacherLeaderboard({ challenge, results, safeRows }) {
  const rows = safeRows??buildSafeClassLeaderboard(challenge, results);
...
export default function ClassChallengesTeacher({ school = null, challenges = [], results = [], onCreate = () => ({ ok: false }), onFinish = () => ({ ok: false }) }) {
  const [creating, setCreating] = useState(false);
  const classOptions=school?.classes??CLASS_OPTIONS;
  const defaultDraft={...DEFAULT_DRAFT,...(school?{classId:classOptions[0]?.id??"",classLabel:classOptions[0]?.label??"",level:classOptions[0]?.level??""}:{})};
  ...
  const questions = useMemo(() => school?school.questionMetadata:readQuestionBankQuestions(), [school?.questionMetadata]);
  const banks = useMemo(() => school?school.banks:getPublishedClassChallengeBanks(questions), [questions,school?.banks]);
  ...
  const selected = school?school.detail?.challenge??null:challenges.find((item) => item.id === selectedId) || challenges[0] || null;
  const statusOf=challenge=>school?challenge.status:getClassChallengeStatus(challenge);
  const liveCount = school?.counts?.active??challenges.filter((item) => statusOf(item) === "en_cours").length;
  const finishedCount = school?.counts?.finished??challenges.length - liveCount;
  const totalParticipants = school?.counts?.participants??new Set(results.filter((item) => challenges.some((challenge) => challenge.id === item.challengeId)).map((item) => item.participantId)).size;
  ...
  const submit = async (event) => {
    ...
    const outcome = await onCreate(validation.value);
    if (!outcome?.ok) {
