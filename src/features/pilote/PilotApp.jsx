...
export default function PilotApp(){
  ...
  async function refresh({reset=false}={}){
    ...
    try{
      ...
      if(next.authenticated){
        if(next.user.role==='admin'){window.location.replace('/admin/ecoles-acces');return {status:'redirect'};}
  ...
  return <div className="pilot-app"><a className="pilot-skip" href="#pilot-main">Aller au contenu</a>
    ...
    {!user&&session?.signInPath&&<section className="pilot-panel"><h2>Retrouver mon espace</h2><p>Votre identité est vérifiée par le service de connexion de l’établissement. Seuls les comptes associés à une école peuvent accéder aux travaux.</p><a className="button button-dark" href={session.signInPath}>Me connecter <ArrowRight/></a></section>}
