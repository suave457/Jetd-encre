import { Question, ShieldCheck, SignOut, Sparkle, ArrowLeft, BookOpenText, Trophy, NotePencil } from '@phosphor-icons/react/ssr';
...
const roleLabels={eleve:'Élève',parent:'Parent',enseignant:'Enseignant',directeur:'Direction',admin:'Administration'};
const studentAvatarIcons={sparkle:Sparkle,book:BookOpenText,trophy:Trophy,pencil:NotePencil};
export const SchoolRoleContext=createContext(null);
...
export default function SchoolShell({role,user,schoolName,nav=[],activeId,onLogout,busy,xp,children,onNavigate,mainId='pilot-main',secondaryNav}) {
  const roleNav=nav.map(item=>({...item,href:withSchoolProfile(item.href,role)}));
  const Avatar=role==='eleve'?studentAvatarIcons[user?.avatar]:null;
  const helpPath=role==='admin'?'/guide-ecole':withSchoolProfile('/pilote?section=aide',role);
  ...
  return <SchoolRoleContext.Provider value={role}><div className={`app-shell school-shell school-role-${role}`}>
    ...
    <aside className="app-sidebar" aria-label={`Menu ${roleLabels[role]||'scolaire'}`}>
      ...
      <nav aria-label={role==='admin'?'Navigation administrateur':`Navigation ${roleLabels[role]||'scolaire'}`}>{roleNav.map(({id,label,href,Icon,disabled,note})=>disabled?<span key={id} className="side-link school-nav-unavailable" aria-disabled="true">{Icon&&<Icon/>}<span>{label}{note&&<small>{note}</small>}</span></span>:<a key={id} className={`side-link ${activeId===id?'active':''}`} aria-current={activeId===id?'page':undefined} href={href} onClick={e=>follow(e,href)}>{Icon&&<Icon weight={activeId===id?'fill':'regular'}/>}<span>{label}{note&&<small>{note}</small>}</span></a>)}</nav>
      {secondaryNav&&<div className="school-secondary-nav">{secondaryNav}</div>}
      <a className="sidebar-help" href={helpPath} onClick={role==='admin'?undefined:e=>follow(e,helpPath)}><Question weight="fill"/><span><strong>Aide & assistance</strong><small>Guides et connexion</small></span></a>
      {onLogout&&<button className="side-logout" onClick={onLogout} disabled={busy}><SignOut/> Se déconnecter</button>}
    </aside>
    <div className="app-area"><header className="app-topbar school-topbar"><span className="school-name">{schoolName||user.schoolName||'Administration Jet d’Encre'}</span><div className="topbar-meta">{typeof xp==='number'&&<span className="student-xp-chip"><Sparkle weight="fill"/><strong>{xp} XP</strong></span>}<span className="demo-badge"><ShieldCheck weight="fill"/> Site de test</span><div className="profile-button school-identity"><span aria-hidden="true">{Avatar?<Avatar size={24} weight="duotone"/>:initials}</span><span><strong>{user.name}</strong><small>{roleLabels[role]}</small></span></div></div></header>{children}</div>
  </div></SchoolRoleContext.Provider>;
}
