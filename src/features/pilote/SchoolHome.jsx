...
export function schoolMenu(role){
  ...
  if(role==='eleve')items.push({id:'jeux',label:'Mes jeux',Icon:GameController});
...
export function SchoolLink({to,onNavigate,children,...props}){
  return <a href={to} {...props} onClick={event=>{if(event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();onNavigate(to);}}}>{children}</a>;
}
...
export default function SchoolHome({user,workspace,onNavigate,onCreate,summary}){
  ...
  return <div className="school-dashboard">
    ...
    <div className="school-dashboard-grid"><section className="panel"><span className="page-eyebrow">{teacher?'TRAVAUX DE MES CLASSES':'MES TRAVAUX'}</span><h2>{latest?.title||(teacher?'Le premier devoir de ma classe':'Aucun devoir pour le moment')}</h2><p>{latest?`À remettre pour le ${new Date(latest.dueDate+'T12:00:00').toLocaleDateString('fr-MA')}`:teacher?'Choisis une classe et prépare une consigne pour tes élèves.':'Les travaux apparaîtront ici dès leur publication par l’enseignant.'}</p><p>{counts.submitted} {counts.submitted===1?'travail remis':'travaux remis'} · {counts.reviewed} {counts.reviewed===1?'retour disponible':'retours disponibles'}</p>{teacher&&!latest?<button className="button button-dark" onClick={onCreate}>Créer un devoir <ArrowRight/></button>:<SchoolLink to={latestPath} onNavigate={onNavigate} className="button button-light">{latest?'Ouvrir le devoir':'Consulter les devoirs'}<ArrowRight/></SchoolLink>}</section>
