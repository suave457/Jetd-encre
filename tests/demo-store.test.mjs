...
test("exposes one working demo session for every role", () => {
  ...
  for (const role of DEMO_ROLES) {
    const result = store.actions.signIn(role);
  ...
  assert.equal(store.actions.signIn("admin", { password: "incorrect" }).error, "invalid_credentials");
});
...
test("links a pending activation to the student profile at sign-in", () => {
  ...
  const signedIn = store.actions.signIn("eleve");
...
test("persists the complete assignment submission and correction loop", () => {
  ...
  store.actions.signIn("enseignant");
